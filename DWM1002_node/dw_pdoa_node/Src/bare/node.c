...
error_e initiator_send_blink(void const *argument)
{
    ...
    // Initiator Sends Blink to Responder
    txPckt.psduLen              = sizeof(blink_msg_t);
    txPckt.delayedTxTimeH_sy    = 0;
    // There shouldn't be an expectation for response 
    // txPckt.txFlag               = ( DWT_START_TX_IMMEDIATE | DWT_RESPONSE_EXPECTED );
    txPckt.txFlag               =  DWT_START_TX_IMMEDIATE ;
    txPckt.delayedRxTime_sy     = 0;  //activate receiver this time SY after Blink Tx
    txPckt.delayedRxTimeout_sy  = 0;     //receiver will be active this time, SY
    pTxMsg->frameCtrl[0]        = Head_Msg_BLINK;
    pTxMsg->seqNum              = (uint8_t)0x01;
    // //memcpy(&pTxMsg->tagID, &p->eui64, sizeof(pTxMsg->tagID));
    // Node doesn't have a tag ID, just set it manually
    uint8_t fixedTagID[8] = {0x12, 0x34, 0x56, 0x78, 0x9A, 0xBC, 0xDE, 0xF0};
    memcpy(pTxMsg->tagID, fixedTagID, sizeof(fixedTagID));
