 ...
 * */
#define WKUP_RESOLUTION_US                  (1e6f/32768.f)
...
static void twr_configure_rtc_wakeup_us(uint32_t     period_us);
...
error_e tag_process_init(void)
{
    ...
    // initialise pseudo-random sequence generator with chip-dependent seed
    srand((unsigned int)dwt_getpartid());
    ...
    switch (fcode)
    {
        ...
        case Twr_Fcode_Resp_Ext:
                ...
                pTwrInfo->env.nodeAddr[1] == ((resp_ext_msg_t*)pMsg)->mac.sourceAddr[1])
            {
                ...
                uint32_t        nextWakeUpPeriod_us, rtcNow, tmpNow;

                // casting received bytes to the int,
                pTwrInfo->env.slotCorr_us = (int32_t)AR2U32(((resp_ext_msg_t*)pMsg)->resp.slotCorr_us);
                ...
                nextWakeUpPeriod_us  = pTwrInfo->env.sframePeriod_us 
                                    -(WKUP_RESOLUTION_US * tmpNow)
                                    - pTwrInfo->env.slotCorr_us;

                if(nextWakeUpPeriod_us < (pTwrInfo->env.sframePeriod_us >>1))
                {
                    nextWakeUpPeriod_us += (pTwrInfo->env.sframePeriod_us);
                }

                twr_configure_rtc_wakeup_us( nextWakeUpPeriod_us );
...
error_e initiator_received_ranging_config(rx_pckt_t *pRxPckt, twr_info_t *pTwrInfo)
{
    ...
    if( ret == _NO_ERR)
    {
        ...
        pTwrInfo->env.sframePeriod_us       = 1000 * (uint32_t)(AR2U16(pRxMsg->sframePeriod_ms));
        ...
        pTwrInfo->env.slotCorr_us           = (int32_t)AR2U32(pRxMsg->slotCorr_us);              //next wakeup correction to fit the slot
...
error_e initiator_received_response(rx_pckt_t *prxPckt, twr_info_t *p)
{
    ...
    final_msg_imuData_t      *pTxMsg      = &TxPckt.msg.finalMsg;

    /* Construct TxPckt packet: no reception after Final transmission */
    TxPckt.psduLen                = sizeof(final_msg_imuData_t);
 ...
 * */
static void twr_configure_rtc_wakeup_us(uint32_t     period)
{
    ret_code_t           err_code;
    ...
    gRTC_SF_PERIOD = (period / WKUP_RESOLUTION_US);
    ...
    uint32_t old_cc = nrf_drv_rtc_counter_get(&rtc);
    uint32_t new_cc = old_cc + gRTC_SF_PERIOD;
    new_cc &= 0xFFFFFF;  // 24-bit only counter
    err_code = nrfx_rtc_cc_set( &rtc, 0, new_cc, true);
    APP_ERROR_CHECK(err_code);
...
static void rtcWakeUpTimerEventCallback(nrf_drv_rtc_int_type_t int_type)
{
    ...
    uint32_t new_cc = current_counter + gRTC_SF_PERIOD; 
    new_cc &= 0xFFFFFF;
    ...
    nrf_drv_rtc_enable(&rtc);
      ...
      {
        pTwrInfo->gRtcSFrameZeroCnt = current_counter;

        if(count == 0)
        {
            twr_configure_rtc_wakeup_us(pTwrInfo->env.sframePeriod_us);
        }
